...
try:
    while True:
        ...
        xDist = distance*math.cos(angle*math.pi/180) + imuPos[2];
        # Make LiDAR y negative to account for the IMU setup
        yDist = -distance*math.sin(angle*math.pi/180) + imuPos[1];
        # z coordinate will be ignored for now
        oFile.write(str(xDist) + "," + str(yDist) + "," + str(imuPos[2]) + "," + str(imuPos[1]) + "\n");
